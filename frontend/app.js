...
function renderChart(data) {
    ...
    const realPrices = data.map(item => item.real);
    ...
    myStockChart = new Chart(chartCanvas, {
        ...
        options: { 
            ...
            scales: {
                x: { type: 'time', time: { unit: 'year' }, grid: { color: 'rgba(255, 255, 255, 0.1)' }, ticks: { color: 'rgb(209, 213, 219)' } },
...
async function handleFetchAndTrain() {
    ...
    setTrainingStatus(`Downloading last 2 years of ${ticker} data & training...`, 'training');
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (result.training_history && result.training_history.loss) {
                historyHtml += `<div class="history-log">`; 
                historyHtml += `<strong>Epoch History (Loss / Val Loss):</strong>`;
                historyHtml += `<ul class="epoch-list">`;
                
                const losses = result.training_history.loss;
                const val_losses = result.training_history.val_loss;

                for (let i = 0; i < losses.length; i++) {
                    historyHtml += `<li>Epoch ${i+1}: ${losses[i].toFixed(4)} / ${val_losses[i].toFixed(4)}</li>`;
                }
                historyHtml += `</ul></div>`;
            }
